...
$(document).ready(function () {
    ...
    function countdown() {
        ...
        */
        ...
        // update the duration in ms
        ms = then.diff(now, 'milliseconds', true);
        let days = Math.floor(moment.duration(ms).asDays());

        then = then.subtract(days, 'days');
        // update the duration in ms
        ms = then.diff(now, 'milliseconds', true);
        let hours = Math.floor(moment.duration(ms).asHours());

        then = then.subtract(hours, 'hours');
        // update the duration in ms
        ms = then.diff(now, 'milliseconds', true);
        let  minutes = Math.floor(moment.duration(ms).asMinutes());

        then = then.subtract(minutes, 'minutes');
        // update the duration in ms
        ms = then.diff(now, 'milliseconds', true);
        let seconds = Math.floor(moment.duration(ms).asSeconds());

        // concatonate the variables
        let diff = '<div class="num">' + days + ' <span class="text"> Days Left</span></div>';
        $('#daysLeft').html(diff);
    }
